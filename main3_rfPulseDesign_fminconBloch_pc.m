...
[pulseFile,path] = uigetfile('rfPulses\*.mat','Select RF pulse (as .mat file) to use as start pulse for fmincon optimization with Bloch');
load([path,pulseFile]);
...
%     parpool([13,96]);
    options = optimset('Display','iter','Algorithm',algoropt,'UseParallel',true,'OutputFcn',str2func(['outfun_pc']));
