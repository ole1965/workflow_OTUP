%Script to optimize the 2sos basis trajectory using the particle swarm optimization(as described in the manuscript and step 1 in the 'readMe.md').
setUp;
addpath('gradOpt');
...
% location of spiral 1 on the z axis,location of spiral 2]
lb = [0.001 , 0 , 0.001  , 3   ,  0.001 , 0 , 0.001, 3   ,   -100 , -100];
ub = [200   , 5 , 15     , 950 ,    200 , 5 , 15   , 950 ,    100 ,  100];

options = optimoptions('particleswarm','UseParallel',true,'Display','iter');

maxiter = 20;%number of iterations for the lsqr-algorithm inside the cost function ZF_2sos
nr = 20;
...
for k = 1:nr
    [paras(k,:),rmse_val(k)] = particleswarm(@(paras) ZF_2sos(paras,mapB1,spatial,targetVec,maxiter,mapB0),...
                10,lb,ub,options);
    save gradientWaveforms\optParas\results_gradOpt_2sos.mat paras rmse_val
end
...
paras = paras(pos,:);
gr = transformParasToWaveform_2sos(paras);

t = datetime('now','Format','yyyy-MM-dd''T''HHmmssSSS');S = char(t);
save(['gradientWaveforms\gradient_2sos_',targetstr,'_',S,'.mat'],'gr');